...
-- | Reserve coin token
--data R = R deriving (Haskell.Show, Haskell.Eq, Generic)
--PlutusTx.makeIsDataIndexed ''R [('R, 0)]
...
-- | Reserve-contract coin token
data ReserveState = ReserveState
PlutusTx.makeIsDataIndexed ''ReserveState [('ReserveState, 0)]
PlutusTx.makeLift ''ReserveState
...
--newtype Reserve = Reserve
--    { rsCoin :: Coin R
--    } deriving stock    (Haskell.Show, Generic)
--      deriving anyclass (ToJSON, FromJSON, ToSchema)
--      deriving newtype  (Haskell.Eq, Haskell.Ord)
--PlutusTx.makeIsDataIndexed ''Reserve [('Reserve, 0)]
...
data ReserveDatum = Unused LiquidityPool Integer | Used LiquidityPool PubKeyHash Integer
    deriving (Generic, Haskell.Show)
PlutusTx.unstableMakeIsData ''ReserveDatum
PlutusTx.makeLift ''ReserveDatum

instance Eq ReserveDatum where
    Unused lp1 res1 == Unused lp2 res2       = lp1 == lp2 && res1 == res2
    Used lp1 pkh1 res1 == Used lp2 pkh2 res2 = pkh1 == pkh2 && lp1 == lp2 && res1 == res2

data ReserveRedeemer = Reserve | Retrieve | Include | Destroy
    deriving (Generic, Haskell.Show)
PlutusTx.unstableMakeIsData ''ReserveRedeemer
PlutusTx.makeLift ''ReserveRedeemer 
