...
import qualified Reserve.OnChain                         as RS
import qualified Swap                                    as US
...
import           Uniswap                                 (UniswapContracts,
                                                          cidFile)
...
main = do
  ...
  where
    ...
    go cid cs = do
        ...
        case cmd of
            Funds                     -> getFunds cid
            ReserveFunds              -> getReserveFunds cid
            Pools                     -> getPools cid
            ...
            Swap tnA tnB              -> swap cid $ toSwapParams cs tnA tnB -- deleted amtA here, Amount will be taken directly from UTxO value
            Reserve amtA tnA amtB tnB -> reserve cid $ toReserveParams cs amtA tnA amtB tnB
            Retrieve tnA tnB          -> retrieve cid $ toRetrieveParams cs tnA tnB
        go cid cs
...
readCommandIO = do
    putStrLn "Enter a command: Funds, ReserveFunds, Pools, Create amtA tnA amtB tnB, Add amtA tnA amtB tnB, Remove amt tnA tnB, Close tnA tnB, Swap tnA tnB, Reserve amtA tnA amtB tnB, Retrieve tnA tnB"
    s <- getLine
...
toReserveParams :: CurrencySymbol -> Integer -> Char -> Integer -> Char -> US.ReserveParams
toReserveParams cs amtA tnA amtB tnB = US.ReserveParams (toCoin cs tnA) (toCoin cs tnB) (US.Amount amtA) (US.Amount amtB)

toRetrieveParams :: CurrencySymbol -> Char -> Char -> US.RetrieveParams 
toRetrieveParams cs tnA tnB = US.RetrieveParams (toCoin cs tnA) (toCoin cs tnB)
...
toSwapParams :: CurrencySymbol -> Char -> Char -> US.SwapParams
toSwapParams cs tnA tnB = US.SwapParams (toCoin cs tnA) (toCoin cs tnB)
...
reserve :: UUID -> US.ReserveParams -> IO ()
reserve cid rs = do
    callEndpoint cid "reserve" rs
  ...
  where
    go = do
        e <- getStatus cid
        case e of
            Right US.Reserved -> putStrLn "reserved"
            Left err'         -> putStrLn $ "error: " ++ show err'
            _                 -> go

retrieve :: UUID -> US.RetrieveParams -> IO ()
retrieve cid rt = do
    callEndpoint cid "retrieve" rt 
    threadDelay 2_000_000
    go
  where
    go = do
        e <- getStatus cid
        case e of
            Right US.Retrieved -> putStrLn "retrieved"
            Left err'         -> putStrLn $ "error: " ++ show err'
            _                 -> go
...
getReserveFunds :: UUID -> IO ()
getReserveFunds cid = do
    callEndpoint cid "reserveFunds" ()
    threadDelay 2_000_000
    go
  where
    go = do
        e <- getStatus cid
        case e of
            Right (US.ReserveFunds v) -> showFunds v
            _                  -> go
